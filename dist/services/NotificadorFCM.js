...
class NotificadorFCM {
    ...
    /* ----------------------------------------------
       NUEVO INCIDENTE
       ---------------------------------------------- */
    static async notificarNuevoIncidente(inc) {
        try {
    ...
    static async enviarNotificacionPersonalizada(params) {
        try {
            ...
            await firebase_admin_1.default.messaging().sendEachForMulticast(mensaje_config);
        }
        catch (error) {
     ...
     */
    static async enviarNotificacion(notificacion) {
        try {
            const message = {
                notification: {
     ...
     */
    static async suscribirATopico(tokens, topico) {
        try {
            if (tokens.length === 0) {
                console.warn('No hay tokens para suscribir al topico:', topico);
                return;
            }
            await firebase_admin_1.default.messaging().subscribeToTopic(tokens, topico);
        }
        catch (error) {
     ...
     */
    static async desuscribirDeTopico(tokens, topico) {
        try {
            if (tokens.length === 0) {
                console.warn('No hay tokens para desuscribir del topico:', topico);
                return;
            }
            await firebase_admin_1.default.messaging().unsubscribeFromTopic(tokens, topico);
        }
        catch (error) {
    ...
    static async notificarContinuarMovimiento(incidente, comentario) {
        try {
