...
exports.listarIncidentesPorCursor = listarIncidentesPorCursor;
...
class IncidenteModel {
    ...
    static async verificarPeriodoVerificacion(incidenteId) {
        try {
            ...
            incidente_logger_1.incidenteError.error('Error al verificar periodo de verificación', { incidenteId, error });
    ...
    static async procesarImagenes(imagenes, incidenteId) {
        try {
            ...
            incidente_logger_1.incidenteError.error('Error al procesar imágenes', { incidenteId, error });
    ...
    static async reorganizarRondasPorIncidente(empresaId, localidadId, movimientoId) {
        try {
            ...
            if (!rondaMovimiento) {
                incidente_logger_1.incidenteError.info('No se encontró ronda para el movimiento', { movimientoId, empresaId, localidadId });
                return;
            }
            ...
            incidente_logger_1.incidenteError.error('Error al reorganizar rondas por incidente', { empresaId, localidadId, movimientoId, error });
    ...
    static async tamanoDeRonda(tx, localidadId, rondaNumero) {
        if (!Number.isFinite(rondaNumero) || rondaNumero < 1)
            return 0;
        ...
        try {
            ...
            for (const rutaImagen of imagenes) {
                if (!rutaImagen)
                    continue;
                try {
                    await promises_1.default.unlink(path_1.default.join(IMAGEN_CONFIG.carpetaBase, rutaImagen));
                }
                catch (error) {
