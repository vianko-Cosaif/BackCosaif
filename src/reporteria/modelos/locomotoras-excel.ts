...
function fillResumenSheet(ws: ExcelJS.Worksheet, data: LocomotorasReporte, movSheetName: string) {
  ...
  for (const loco of data.locomotoras) {
    const row = ws.addRow([loco.locomotiveNumber, null]);

    // Fórmula: cuenta cuántas veces aparece la locomotora en la columna A de Movimientos
    // Esto permite que, si alguien edita la hoja Movimientos, el resumen se recalcula.
    const formula = `COUNTIF(${movRef}!$A:$A, A${rowIndex})`;
    row.getCell(2).value = { formula, result: loco.totalMovimientos };
