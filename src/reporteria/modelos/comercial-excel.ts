...
function buildDashboard(
  ...
  for (let index = 0; index < charts.length; index += 1) {
    ...
    const chart = addNativeColumnChartData(sheet, {
      ...
      categories: charts[index].data.map((item) => item.label),
      series: [{
        name: 'Total',
        color: charts[index].data[0]?.color || ACCENT,
        pointColors: charts[index].data.map((item) => item.color),
        values: charts[index].data.map((item) => item.value),
      }],
      startColumn: 18 + index * 3,
      startRow: chartRow,
      anchor: {
        fromCol: leftSide ? 0 : 8,
        fromRow: chartRow - 1,
        toCol: leftSide ? 8 : 16,
        toRow: chartRow + 15,
      },
    });
    if (chart) nativeCharts.push(chart);
  }
...
export async function exportarComercialExcel(
  ...
  if (options.sections.resumen || Object.values(options.charts).some(Boolean)) {
    nativeCharts.push(...buildDashboard(workbook, reporte, options));
  }
  ...
  const buffer = await injectNativeExcelCharts(rawBuffer, nativeCharts);
  return {
