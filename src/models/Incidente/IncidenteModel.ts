...
private static async hayOtrasAltasEnRonda1(
  localidadId: number,
    ...
    try {
      ...
      if (prioridad === 'ALTA') {
        ...
        if (otrasAltas) {
          ...
          await this.intercambiarConPrimerBajaDeRonda2(
            localidadId,
            empresaId,
            movimientoId,
            rondaMovimiento
   ...
   * ──────────────────────────────────────────────────────────
   */
  private static async normalizarNumeracionRondas (localidadId: number) {
    await prisma.$transaction(async tx => {
