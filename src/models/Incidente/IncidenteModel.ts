...
export class IncidenteModel {
  ...
  /**
   * Reorganiza las rondas cuando se reporta un incidente en un movimiento específico.
   * REGLA ALTA o RONDA 1: El movimiento se mueve al final de la misma ronda.
   * REGLA BAJA (RONDA 2+): El movimiento se mueve a la siguiente ronda con efecto dominó.
   * Después, renumera todas las rondas de la localidad, reubicando solo los movimientos
   * de la empresa que generó el incidente según su nueva secuencia.
   *
   * @private
   * @param empresaId - ID de la empresa afectada
   * @param localidadId - ID de la localidad donde ocurre el incidente
   * @param movimientoId - ID del movimiento específico con incidente
   */
  public static async reorganizarRondasPorIncidente(
    ...
    try {
      ...
      if (!rondaMovimiento) {
        incidenteError.info('No se encontró ronda para el movimiento', {
          movimientoId,
          empresaId,
          localidadId
        });
      ...
      incidenteError.info('Iniciando reorganización por incidente', {
        movimientoId,
        empresaId,
        rondaOriginal,
        localidadId,
      ...
      if (prioridad === 'ALTA') {
        ...
        if (unicaEmpresa) {
          // Si es la única empresa, solo se reordena internamente
  ...
  /**
   * Aplica efecto dominó para prioridad BAJA:
   * - Incrementa +1 rondaNumero para todos los movimientos afectados
   * - Luego renumera órdenes en cada ronda 1…N
   */
  private static async aplicarEfectoDomino(
    ...
    await prisma.$transaction(async (tx) => {
      incidenteError.info('Iniciando efecto dominó', {
        movimientoId,
        empresaId,
        desdeRonda: desde
      });
      ...
      incidenteError.info('Efecto dominó completado', {
        movimientoId,
        empresaId,
  ...
  /**
   * Renumera los órdenes en una ronda para que queden 1,2,3…
   */
  ...
  private static async moverMovimientoARonda1AlFinal(
    ...
    await prisma.$transaction(async (tx) => {
      ...
      /* 2) calcular el último orden de la ronda 1 en esa localidad */
  ...
  static async crearIncidente(data: {
    ...
    try {
      ...
      /* - 5. mover o reorganizar la ronda según prioridad -------- */
  ...
  /**
   * Notificar cambio de estado de un incidente (ABIERTO ? CERRADO o actualización)
   */
  ...
  static async reorganizarSiEsEmpresaUnica(movimiento: any) {
    ...
    if (esUnica && movimiento.id) {
      // Si es la única empresa, reorganizar sus rondas internamente
      await this.reorganizarRondasPorIncidente(
        movimiento.empresaId,
        movimiento.localidadId,
        movimiento.id
