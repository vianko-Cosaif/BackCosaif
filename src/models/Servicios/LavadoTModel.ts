...
export class LavadoTModel {
  ...
  static async editar(
    ...
    try {
      ...
      movimientoError.error('Error editando LavadoT', { id, input, errName: error?.name, errMsg: error?.message });
  ...
  static async iniciar(id: number, usuarioId?: number, inicio?: Date): Promise<LavadoT> {
    try {
      return await prisma.$transaction(async (tx) => {
        const row = await tx.lavadoT.findUnique({
          where: { id },
  ...
  /** Asignar operador al movimiento. */
  ...
  /** Listar no en proceso (pendientes). */
  ...
  /** Siguientes para iniciar (máx 10). */
