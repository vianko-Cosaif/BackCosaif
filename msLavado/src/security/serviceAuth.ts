...
const parseSecrets = (): Map<string, string> => {
  const raw = process.env.LAVADO_SERVICE_AUTH_SECRETS;
  if (raw) {
  ...
  const serviceId = process.env.LAVADO_SERVICE_ID;
  const secret = process.env.LAVADO_SERVICE_SECRET;
...
const getToleranceMs = () => {
  const value = Number(process.env.LAVADO_SIGNATURE_TOLERANCE_MS);
