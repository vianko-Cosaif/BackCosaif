...
|---|---|---|---|---:|---|
| API principal | `BackCosaif2/src` | Usuarios, empresas, localidades, movimientos generales, rondas, incidentes, FCM, reportes y tiempo real | `DATABASE_URL` — ejemplo `cosaif_local` | 3000 | `GET /` |
| msTorno | `BackCosaif2/msTorno` | Torneado, ruedas, rondas de torno, navajas e incidentes de torno | `TORNO_DATABASE_URL` — ejemplo `torno` | 3002 | `GET /health` |
| ms_torreon | `BackCosaif2/ms_torreon` | Naturales y arrastres de Torreón, vagones, rondas, bloqueos e incidentes | `TORREON_DATABASE_URL` — ejemplo `torreon_development` | 3003 | `GET /health` |
| msComercial | `BackCosaif2/msComercial` | Clientes comerciales, contratos, paquetes, cortes, planes y CRM | `COMERCIAL_DATABASE_URL` — ejemplo `cosaif_comercial` | 3004 | `GET /health` |
| Web | `CosaifWeb1` | Aplicación Next.js, BFF, PWA, Firebase Web Push y paneles por rol | No usa Prisma | 3012 | `GET /login` |
...
| Entorno | Uso esperado | Terminal de los ejemplos |
|---|---|---|
| Ubuntu Server 22.04/24.04 LTS | Producción o pruebas integrales | Bash |
| macOS | Desarrollo local | zsh/Bash |
| Windows 11 | Desarrollo local | PowerShell 7 o Windows PowerShell |

Los comandos con `sudo`, `cp`, `tar`, `$(date ...)` o continuación `\` están escritos para Ubuntu/macOS. Cuando exista diferencia relevante se incluye el equivalente PowerShell. Los comandos `npm run ...` funcionan igual en las tres plataformas.
...
```bash
sudo apt update
sudo apt install -y git postgresql postgresql-client build-essential curl ca-certificates
```

Instale Node.js 20 LTS mediante el mecanismo aprobado para el servidor —por ejemplo, un repositorio corporativo o un administrador de versiones— y confirme que tanto `node` como `npm` estén disponibles para el usuario que ejecutará COSAIF.

Si PostgreSQL está en otro servidor, basta con `postgresql-client`. Si será local, habilite el servicio:
...
### Preparar macOS para desarrollo

Puede instalar los requisitos con su administrador de versiones preferido. Con Homebrew:
...
Si `node@20` no queda primero en `PATH`, siga la indicación que imprime Homebrew o use un administrador de versiones de Node. En Apple Silicon, Homebrew suele vivir en `/opt/homebrew`; en equipos Intel, en `/usr/local`.

### Preparar Windows para desarrollo

1. Instale Node.js 20 LTS.
2. Instale PostgreSQL 15/16 incluyendo Command Line Tools.
3. Instale Git for Windows.
4. Agregue la carpeta `bin` de PostgreSQL al `PATH`, por ejemplo `C:\Program Files\PostgreSQL\16\bin`.
5. Abra una terminal nueva de PowerShell y ejecute las verificaciones de versiones.

WSL2 también es válido y simplifica el uso de comandos Bash. Si usa WSL2, mantenga código y `node_modules` dentro del filesystem Linux para evitar problemas de rendimiento y permisos.
...
En Windows una ruta equivalente puede ser:
...
Todos los microservicios comparten el `package.json` y `node_modules` de `BackCosaif2`.

```bash
cd /ruta/a/eco/BackCosaif2
npm ci
```
...
```powershell
Set-Location C:\Sistemas\eco\BackCosaif2
npm ci
```
...
```powershell
Set-Location C:\Sistemas\eco\CosaifWeb1
npm ci
```
...
- `BackCosaif2/.env`
- `BackCosaif2/msTorno/.env.torno`
- `BackCosaif2/ms_torreon/.env.torreon`
- `BackCosaif2/msComercial/.env.comercial`
- `CosaifWeb1/.env.local`
- credencial Firebase Admin en `BackCosaif2/src/config/cosaifapp-firebase-adminsdk-fbsvc-a3c14d6bfb.json`
...
La API principal también usa `COMERCIAL_SERVICE_ID`, `COMERCIAL_SERVICE_SECRET` y `COMERCIAL_MS_URL`. En desarrollo, si el secreto no existe en `.env`, el cliente interno intenta cargarlo desde `msComercial/.env.comercial` sin mostrarlo ni copiarlo. En producción ese fallback está deshabilitado: el supervisor debe inyectar el secreto explícitamente en la API y en msComercial. Después de cambiar un secreto reinicie ambos procesos.

### Frontend — `CosaifWeb1/.env.local`
...
```powershell
Copy-Item msTorno\generated msTorno\dist\generated -Recurse -Force
Copy-Item ms_torreon\generated ms_torreon\dist\generated -Recurse -Force
Copy-Item msComercial\generated msComercial\dist\generated -Recurse -Force
```
...
Los procesos deben arrancarse desde la raíz `BackCosaif2`. La API principal carga `.env` directamente:
...
### Implementación recomendada en Ubuntu Server

Una ubicación clara para producción es:
...
Cree un usuario de servicio sin acceso administrativo y entregue la carpeta a ese usuario:

```bash
sudo adduser --system --group --home /opt/cosaif cosaif
sudo mkdir -p /opt/cosaif
sudo chown -R cosaif:cosaif /opt/cosaif
```

Después, como usuario `cosaif`:

1. coloque o clone ambos proyectos;
2. configure los cinco archivos de entorno;
3. ejecute `npm ci`;
4. genere los cuatro clientes Prisma;
5. aplique solo los `db push` necesarios;
6. compile los cuatro procesos backend;
7. copie los tres clientes `generated` a sus respectivos `dist`;
8. compile el frontend.

Proteja secretos:

```bash
sudo chown cosaif:cosaif \
  /opt/cosaif/BackCosaif2/.env \
  /opt/cosaif/BackCosaif2/msTorno/.env.torno \
  /opt/cosaif/BackCosaif2/ms_torreon/.env.torreon \
  /opt/cosaif/BackCosaif2/msComercial/.env.comercial \
  /opt/cosaif/CosaifWeb1/.env.local
...
#### Unidad systemd de la API principal

Ejemplo `/etc/systemd/system/cosaif-api.service`:

```ini
[Unit]
Description=COSAIF API principal
After=network-online.target postgresql.service cosaif-torno.service cosaif-torreon.service cosaif-comercial.service
...
User=cosaif
Group=cosaif
WorkingDirectory=/opt/cosaif/BackCosaif2
EnvironmentFile=/opt/cosaif/BackCosaif2/.env
ExecStart=/usr/bin/node /opt/cosaif/BackCosaif2/dist/index.js
Restart=on-failure
RestartSec=5
TimeoutStopSec=30
...
#### Unidad systemd de msTorno

Ejemplo `/etc/systemd/system/cosaif-torno.service`:

```ini
[Unit]
Description=COSAIF msTorno
...
Group=cosaif
WorkingDirectory=/opt/cosaif/BackCosaif2
EnvironmentFile=/opt/cosaif/BackCosaif2/msTorno/.env.torno
ExecStart=/usr/bin/node -e "require('./msTorno/dist/src/Servidor').iniciarServidorTorno()"
Restart=on-failure
RestartSec=5
...
Para `cosaif-torreon.service` y `cosaif-comercial.service` use la misma estructura, cambiando `EnvironmentFile` y `ExecStart` por los comandos compilados documentados anteriormente. Para la web use `WorkingDirectory=/opt/cosaif/CosaifWeb1` y `ExecStart=/usr/bin/npm run start`.

Active y revise sin ocultar errores:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now cosaif-torno cosaif-torreon cosaif-comercial cosaif-api
sudo systemctl status cosaif-torno cosaif-torreon cosaif-comercial cosaif-api
sudo journalctl -u cosaif-api -n 100 --no-pager
```

Antes de habilitar la web, confirme los cuatro healthchecks locales. Coloque Nginx, Caddy o el proxy institucional frente a la aplicación y termine TLS allí.
...
PowerShell usa la misma herramienta PostgreSQL con otra sintaxis de variables:

```powershell
$stamp = Get-Date -Format "yyyy-MM-dd_HH-mm"
$folder = Join-Path $HOME "backups\cosaif\$((Get-Date).ToString('yyyy-MM-dd'))"
New-Item -ItemType Directory -Force -Path $folder | Out-Null

pg_dump --format=custom --no-owner --no-acl `
  --dbname="$env:DATABASE_URL" `
  --file=(Join-Path $folder "cosaif_$stamp.dump")
```

Repita el comando sustituyendo `DATABASE_URL` y el prefijo por `TORNO_DATABASE_URL`, `TORREON_DATABASE_URL` y `COMERCIAL_DATABASE_URL`.
...
```bash
tar -xzf /ruta/uploads_FECHA.tar.gz -C /ruta/a/BackCosaif2
```
...
Si aparece `COMERCIAL_SERVICE_SECRET no configurado`, no significa que la base esté caída: la API principal no encontró la llave necesaria para firmar la llamada. En desarrollo confirme que exista `msComercial/.env.comercial` y reinicie la API. En Ubuntu/producción agregue la variable al `EnvironmentFile` de `cosaif-api.service` y al de `cosaif-comercial.service`, ejecute `systemctl daemon-reload` y reinicie ambos servicios.
